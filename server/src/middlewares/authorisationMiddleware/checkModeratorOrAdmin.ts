import { Request, Response, NextFunction } from 'express';
import User from '../../models/User';

// Assure-toi que req.user correspond à ton type après l'authentification JWT
const checkModeratorOrAdmin = (req: Request, res: Response, next: NextFunction) => {
    const allowedRoles = ['moderator', 'admin'];

    if (!req.user) {
        return res.status(403).json({ message: 'User is not authenticated.' });
    }

    // Cast explicite pour que TypeScript comprenne que req.user est de type User
    const user = req.user as User;


    // Vérifie si l'utilisateur a l'un des rôles autorisés
    if (!allowedRoles.includes(user.role)) {
        return res.status(403).json(
            {
                error: [
                    {
                        type: "unauthorized",
                        value: "Unauthorized",
                        msg: "Access denied. This resource is only accessible for moderators or admins.",
                        location: "body"
                    }
                ]
            }
        );
    }

    next();
};

export default checkModeratorOrAdmin;
